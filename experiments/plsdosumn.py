...
# Prevent HuggingFace from hitting network on second SentenceTransformer init
...
def main():
    ...
    for e in bench:
        if e['repo_id'] in ['scikit-learn__scikit-learn']:
    ...
    # Results storage
    all_results = {}
    # baseline_mcts removed — hangs on LLM calls, would take hours
    method_names = ['dense', 'bm25', 'hybrid', 'puct_mcts', 'puct_adapted']
    ...
    for repo_id, entries in test_entries_by_repo.items():
        ...
        for method in method_names:
            ...
            for qi, entry in enumerate(entries):
                ...
                print(f"    [{qi+1}/{len(entries)}] {query[:60]}..." if len(query) > 60 else f"    [{qi+1}/{len(entries)}] {query}")
