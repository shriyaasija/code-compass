...
async def startup_event():
    ...
    try:
        ...
        print(f"\n❌ STARTUP FAILED: {e}")
        if llm_provider == "lmstudio":
            print("Make sure LM Studio server is running (Local Server tab)")
        else:
...
class InitializeRequest(BaseModel):
    ...
    json_tree_path: Optional[str] = None  # Optional - auto-generated if not provided
...
async def initialize_repository(request: InitializeRequest):
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print(f"📥 INITIALIZE REQUEST")
        print(f"   repo_id: {request.repo_id}")
        print(f"   github_url: {request.github_url}")
        print(f"   repo_path: {request.repo_path}")
        print(f"   json_tree_path: {request.json_tree_path}")
        ...
        # Get repository path — prefer repo_path over github_url if both provided
        if request.repo_path:
            ...
            if repo_id in repo_paths and Path(repo_paths[repo_id]).exists():
        ...
        # Validate or generate paths
        if not Path(repo_path).exists():
            print(f"❌ Repository path not found: {repo_path}")
            raise HTTPException(404, f"Repository path not found: {repo_path}")
        
        json_tree_path = request.json_tree_path
        if not json_tree_path or not Path(json_tree_path).exists():
            if request.github_url or not json_tree_path:
                print(f"⚙️ No valid JSON tree path provided. Auto-generating one for {repo_id}...")
                
                # Check if we have the parser
                global code_parser
                ...
                with open(json_tree_path, 'w') as f:
        ...
        chatbots[repo_id] = ProductionChatbot(repo_path, llm_provider=llm_provider, llm_model=llm_client.model)
...
async def health_check():
    ...
    llm_status = "connected" if llm_client else "not initialized"
